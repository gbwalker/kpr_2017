...
# 94 percent of 200 randomly selected posts in total. [N.B., the paper says 93 percent.] It's strange that there are no examples of 1, 2, or 6 (other). How does ReadMe know what these look like if there are no examples of them?

# Since it appears that ReadMe2 does not allow for zero-length categories, I assign 
...
# Keep only the first 300,000 words out of 2 million for computational speed.

wv_split <- separate(wv[1:150000,], value, into = as.character(seq(1:300)), sep = " ", convert = TRUE)
wv_split2 <- separate(wv[150001:300000,], value, into = as.character(seq(1:300)), sep = " ", convert = TRUE)
wv_split_all <- rbind(wv_split, wv_split2)

# Save the file for future use.

write_rds(wv_split_all, "wv_split.rds")

# Read in the saved file.

setwd("C:/Users/Gabriel/Documents/R/win-library/3.5/readme")
wv_split_all <- read_rds("wv_split_all.rds")
wv_small <- wv_split_all[1:100000,]

# Turn the word vectors into a matrix that readme::undergrad() can use.

wv_dm <- as.matrix(wv_small[,2:300])
row.names(wv_dm) <- wv_small$`1`

# Create a document feature matrix.

dfm <- undergrad(documentText = df$text, wordVecs = wv_dm)
# write_rds(dfm, "dfm_100k.rds")
# dfm <- read_rds("dfm_100k.rds")
...
# Save the results to create something of a confidence interval.
...
post <- posterior(mod_lda_bi)
terms(mod_lda_bi)
topics <- topics(mod_lda_bi) %>% 
  as_tibble()

# Show the percentage breakdown of the 5 topics.

count(topics, value) %>% mutate(percent = n / nrow(topics))

###################################################
# K-MEANS CLUSTERING ON THE DOCUMENT FEATURE MATRIX
###################################################

# Use k-means clustering for k = 5 on the dfm produced by undergrad().

mod_kmeans <- kmeans(dfm,
                     centers = 5,
                     algorithm = "Hartigan-Wong")

# Get the category results of the clustering.

results_kmeans <- as_tibble(mod_kmeans$cluster)
count(results_kmeans, value) %>% mutate(percent = n / nrow(results_kmeans))

# Merge the assigned categories into the dataframe...

###################################################
# HIERARCHICAL CLUSTERING ON THE DOCUMENT FEATURE MATRIX
###################################################

# Calculating distances on a dataset this large is not feasible.

mod_hc <- agnes(dfm[1:5000,],
                diss = FALSE,
                metric = "euclidean",
                method = "average",
                stand = FALSE,
